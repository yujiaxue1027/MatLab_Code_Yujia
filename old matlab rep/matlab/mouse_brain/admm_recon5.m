function [output,costlist,out_paras] = admm_recon5(y,psf,para)
...
H1 = @(x) (Ft(F(x).*Hs1));
H2 = @(x) (Ft(F(x).*Hs2));
HT = @(x) (Ft(F(x(:,:,1)).*Hs1_conj)+Ft(F(x(:,:,2)).*Hs2_conj));
HTH = abs(Hs1.*Hs1_conj+Hs2.*Hs2_conj);

xt = zeros(2*rows,2*cols);
gamma1 = zeros(2*rows,2*cols,2);
...
v_mask = 1./(CT(ones(size(y,1),size(y,2))) + mu1); %Denominator of v update (itself is diagonized)

iter = 0;
Hxtp = zeros(2*rows,2*cols,2);
figure;
while iter < maxiter
   iter = iter + 1;
   Hxt = Hxtp;
   vtp(:,:,1) = v_mask.*(mu1*(gamma1(:,:,1)/mu1 + Hxt(:,:,1)) + CTy1);
   vtp(:,:,2) = v_mask.*(mu1*(gamma1(:,:,2)/mu1 + Hxt(:,:,2)) + CTy2);
   ...
   xtp_numerator = mu1*HT(vtp-gamma1/mu1)+...
       ...
       mu3*(wtp-gamma3/mu3)+...
       mu4*(ztp - gamma4/mu4);
   part1 = mu1*HT(vtp-gamma1/mu1);
   part2 = mu2*PsiT(ut1-gamma2_1/mu2,ut2-gamma2_2/mu2);
   part3 = mu3*(wtp-gamma3/mu3);
   part4 = mu4*(ztp - gamma4/mu4);
   
   val1 = norm(norm(norm(mu1*HT(vtp-gamma1/mu1))));
   ...
   residual = abs(C(H1(xt))-y(:,:,1))+abs(C(H2(xt))-y(:,:,2));
   dterm = 0.5*sum(residual(:).^2);
   [tv_x,tv_y] = Psi(xt);
   tvterm = tau1*sqrt(sum(tv_x(:).^2)+sum(tv_y(:).^2));
   l1term = tau2*sum(abs(xt(:)));
