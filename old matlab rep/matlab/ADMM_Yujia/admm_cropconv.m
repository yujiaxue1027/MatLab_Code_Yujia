...
% apply admm to solve: xhat = argmin 0.5*|CHx-y|^2+tau*|Psix|_1+nonnega_indicator{x}

%% load prepared data (groundtruth, y, psf)
data_file_name = 'target_data1.mat';
load(data_file_name);
[rows,cols] = size(psf);


%% ADMM parameters
tau = 10; % sparsity parameter default: 1
mu1 = 0.001; % parameter for v:Hx default: 0.005
mu2 = 0.05; % parameter for u: Psix
mu3 = 0.5; % parameter for w: x
maxiter = 500;

%% define operators
F = @(x) fftshift(fft2(ifftshift(x)));
Ft = @(x) fftshift(ifft2(ifftshift(x)));
C = @(x) x(1+128:256+128,1+128:256+128);
CT = @(x) padarray(x,[128,128]);
...
x_mask = 1./(mu1*HTH + mu2*PsiTPsi + mu3);  %Denominator of x update (diagonized in FFT domain)
...
while iter < maxiter
   ...
   figure(66),subplot(1,2,1),imagesc(xt),colormap gray;axis image;colorbar;title(iter);
   residual = C(H(xt))-y;
   mse = norm(residual(:));
