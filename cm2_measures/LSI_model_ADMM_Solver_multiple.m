function [output,history] = LSI_model_ADMM_Solver_multiple(y,psf,para, varargin)
...
%    disp(['val1: ',num2str(val1),'  val2: ',num2str(val2),'  val3: ',num2str(val3),'  val4: ',num2str(val4)]);
   ...
   tmp_part1todisplay = abs(Ft(F(tmp_part1).*x_mask));
   tmp_part2todisplay = abs(Ft(F(tmp_part2).*x_mask));
   tmp_part3todisplay = abs(Ft(F(tmp_part3).*x_mask));
   tmp_part4todisplay = abs(Ft(F(tmp_part4).*x_mask));
