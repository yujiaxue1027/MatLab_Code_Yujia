%% script to process 3D measurements
...
rows = 1944;
cols = 2592;
y1 = im2double(imread('y1.tif'));
y2 = im2double(imread('y2.tif'));

%% load in all PSFs
num_psf = 701;
psf_stack = zeros(rows,cols,num_psf);
for i = 1:num_psf
    tmp = im2double(imread(['psfs/',num2str(i,'%.2d'),'.tif']));
...
% for i = 200:10:num_psf
%     imwrite(crop2d(uint8(255*linear_normalize(deconv_tik(y1,psf_stack(:,:,i),0.1)))),['temp/y1_',num2str(i),'.png']);
% end
% for i = 1:30:num_psf
%     imwrite(crop2d(uint8(255*linear_normalize(deconv_tik(y2,psf_stack(:,:,i),0.1)))),['temp/y2_',num2str(i),'.png']);
% end
