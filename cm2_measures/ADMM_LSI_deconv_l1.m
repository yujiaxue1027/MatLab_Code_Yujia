function [output,history] = ADMM_LSI_deconv_l1(y,psf,para, varargin)
...
%    wtp = max(clip_min, xt+gamma3/mu3);
%    ztp = wthresh(gamma4/mu4 + xt,'s',tau2/mu4);
   ztp = wthresh(gamma4/mu4 + xt,'s',tau/mu4);
%    
...
%        tmp_part3+...
%        tmp_part4;
   xtp_numerator = tmp_part1 + tmp_part3 + tmp_part4;
...
%    gamma2_2 = gamma2_2 + mu2*(Psixt2-ut2);
   ...
   %Update filters
    if mu1_update || mu3_update || mu4_update
        mu_update = 1;
    else
        mu_update = 0;
    end
    ...
    if mu_update
        disp(['mu updated. mu1: ',num2str(round(mu1,3)), ', mu3: ',num2str(round(mu3,3)), ', mu4: ',num2str(round(mu4,3))]);
        x_mask = 1./(mu1*HTH + mu3 + mu4);
   ...
   if para.display_flag
    ...
    %    loss_mu2 = 0.5 * mu2 * (sum(VEC(loss_mu2_1.^2)) + sum(VEC(loss_mu2_2.^2)));
       ...
       loss_mu4 = xtp - ztp + gamma4/mu4;
       loss_mu4 = 0.5 * mu4 * sum(VEC(loss_mu4.^2));
       loss_tot = loss_cvy + loss_tau + loss_mu1 + loss_mu3 + loss_mu4; 
       disp(['iter: ',num2str(iter),', cvy: ',num2str(round(loss_cvy,3)), ', tau: ',num2str(round(loss_tau,3))...
           ,', mu1: ',  num2str(round(loss_mu1,3))...
           , ', mu3: ',  num2str(round(loss_mu3,3)), ', mu4: ',  num2str(round(loss_mu4,3))...
           , ', total: ',  num2str(round(loss_tot,3))]);
   end
end
