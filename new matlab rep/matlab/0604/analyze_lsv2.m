...
[src_h, src_v] = meshgrid(linspace(-7.5,7.5,num_scans));
...
window = 256;
center_pos = [961, 1308]; %% position of center psf of center scanning position
right_pos = [956, 1794]; %% position of center psf of most right scanning position
down_pos = [1438, 1310]; %% position of center psf of most down scanning position
tmp_separation = 6;
...
right_pos2 = [969, 1913]; %% position of right psf of center scanning position
down_pos2 = [1570, 1301]; %% position of down psf of center scanning position
...
for i = 1:3
    for j = 1:3
        ...
        [J,~] = gray2ind(img);
        imwrite(J,parula,['mla2/lsv_',num2str(i),num2str(j),'.tif']);
    end
end
...
img = img./max(img(:));
[J,~] = gray2ind(img);
imwrite(J*4,parula,['mla2/lsv_psf.tif']);
...
for i = 1:3
    for j=1:3
        ...
        imwrite(J,parula,['mla2/xcorr_map_',num2str(i),num2str(j),'.tif']);
    end
end
%% separation analysis
...
dist_h = dist_h(11:21,11:21);
dist_v = dist_v(11:21,11:21);
