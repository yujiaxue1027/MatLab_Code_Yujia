...
window = 384;
center_pos = [961, 1307]; %% position of center psf of center scanning position
right_pos = [954, 1793]; %% position of center psf of most right scanning position
down_pos = [1436, 1309]; %% position of center psf of most down scanning position
tmp_separation = 6;
...
right_pos2 = [966, 1912]; %% position of right psf of center scanning position
down_pos2 = [1569, 1301]; %% position of down psf of center scanning position
...
for i = 1:3
    for j = 1:3
        ...
        [J,~] = gray2ind(img);
        imwrite(J,parula,['mla2_10mm/lsv_',num2str(i),num2str(j),'.tif']);
    end
end
...
img = img./max(img(:));
[J,~] = gray2ind(img);
imwrite(J*4,parula,['mla2_10mm/lsv_psf.tif']);
...
for i = 1:3
    for j=1:3
        ...
        imwrite(J,parula,['mla2_10mm/xcorr_map_',num2str(i),num2str(j),'.tif']);
    end
end
%% separation analysis
