...
rows = 1944;
cols = 2592;
[src_h, src_v] = meshgrid(linspace(-half_scan_range,half_scan_range,num_scans));
center_pos = (num_scans + 1)/2;
central_img = im2double(imread([path,num2str(center_pos,'%.2d'),'_',num2str(center_pos,'%.2d'),'.tif']));
right_img = im2double(imread([path,num2str(center_pos,'%.2d'),'_',num2str(num_scans,'%.2d'),'.tif']));
down_img = im2double(imread([path,num2str(num_scans,'%.2d'),'_',num2str(center_pos,'%.2d'),'.tif']));
...
window = 256;
center_pos = [966, 1299]; %% position of center psf of center scanning position
right_pos = [968, 1372]; %% position of center psf of most right scanning position
down_pos = [1030, 1302]; %% position of center psf of most down scanning position
tmp_separation = 44;
...
right_pos2 = [976, 1890]; %% position of right psf of center scanning position
down_pos2 = [1566, 1293]; %% position of down psf of center scanning position
            ...
            for jj = 1:3
                ...
                if (centers(1,i,j,ii,jj) - k_size/2)>0 && (centers(2,i,j,ii,jj) - k_size/2)>0 && ...
                    ...
                    psf_stack(:,:,i,j,ii,jj) = data(centers(1,i,j,ii,jj)-k_size/2:centers(1,i,j,ii,jj)+k_size/2-1,...
                        centers(2,i,j,ii,jj)-k_size/2:centers(2,i,j,ii,jj)+k_size/2-1);
                end
            end
        end
    end
...
%         end
%         [J,~] = gray2ind(img);
%         imwrite(J,parula,['mla_R10S51_2/lsv_',num2str(i),num2str(j),'.tif']);
%     end
% end
% % 
...
% img = img./max(img(:));
% [J,~] = gray2ind(img);
% imwrite(J*4,parula,['mla_R10S51_2/lsv_psf.tif']);
    ...
    for j = 1:3
        ...
        for ii = 1:num_scans
            for jj=1:num_scans
                ...
                psf1 = psf1./norm(psf1(:));
                correlation_map(ii,jj,i,j) = compute_pcc(psf0, psf1);%max(max(xcorr2(psf0,psf1)));
            end
        end
        disp(j);
    end
...
for i = 1:3
    for j=1:3
        ...
        imwrite(J,parula,['mla_R1S99/xcorr_map_',num2str(i),num2str(j),'.tif']);
    end
    disp(i);
end
% %% separation analysis
