...
window = 384;
center_pos = [969, 1293]; %% position of center psf of center scanning position
right_pos = [965, 1778]; %% position of center psf of most right scanning position
down_pos = [1446, 1296]; %% position of center psf of most down scanning position
tmp_separation = 6;
...
right_pos2 = [978, 1898]; %% position of right psf of center scanning position
down_pos2 = [1578, 1286]; %% position of down psf of center scanning position
...
for i = 1:3
    for j = 1:3
        ...
        [J,~] = gray2ind(img);
        imwrite(J,parula,['mla2_p10/lsv_',num2str(i),num2str(j),'.tif']);
    end
end
...
img = img./max(img(:));
[J,~] = gray2ind(img);
imwrite(J*4,parula,['mla2_p10/lsv_psf.tif']);
...
for i = 1:3
    for j=1:3
        ...
        imwrite(J,parula,['mla2_p10/xcorr_map_',num2str(i),num2str(j),'.tif']);
    end
end
%% separation analysis
