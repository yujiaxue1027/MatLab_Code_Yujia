...
window = 256;
center_pos = [963, 1303]; %% position of center psf of center scanning position
right_pos = [956, 1811]; %% position of center psf of most right scanning position
down_pos = [1456, 1307]; %% position of center psf of most down scanning position
tmp_separation = 14;
...
right_pos2 = [970, 1892]; %% position of right psf of center scanning position
down_pos2 = [1562, 1296]; %% position of down psf of center scanning position
...
for i = 1:3
    for j = 1:3
        img = zeros(rows,cols);
        for ii = 1:2:num_scans
            for jj=1:2:num_scans
        ...
        [J,~] = gray2ind(img);
        imwrite(J,parula,['mla2_p5_s41/lsv_',num2str(i),num2str(j),'.tif']);
    end
end
...
img = img./max(img(:));
[J,~] = gray2ind(img);
imwrite(J*4,parula,['mla2_p5_s41/lsv_psf.tif']);
...
for i = 1:3
    for j=1:3
        ...
        imwrite(J,parula,['mla2_p5_s41/xcorr_map_',num2str(i),num2str(j),'.tif']);
    end
end
%% separation analysis
